...
module M = Owl_computation_cpu_engine.Make (N)
...
let make_network () = 
  ...
  |> lambda (fun x -> Maths.((tanh x) * (pack_flt 256.) + (pack_flt 127.5)))
  |> get_network
...
let htb_to_weight htbfile weightfile = 
  ...
  Array.iter (fun n ->
    ...
    if nname = conv2d_n then (
      ...
      Printf.printf "%s\n" n.name; 
      wb.(0) <- pack (Hashtbl.find h n.name);
      Neuron.update n.neuron wb
    ) else if nname = norm_n then (
      Printf.printf "%s\n" n.name; 
      let be_ga = Neuron.mkpar n.neuron in
      be_ga.(0) <- pack (Hashtbl.find h (n.name ^ "_beta"));
      be_ga.(1) <- pack (Hashtbl.find h (n.name ^ "_gamma"));
...
let _ = 
  Array.iter (fun style_name -> 
    ...
    let owl = "fst_" ^ style_name ^ "_cg.weight" in
    (* h5_to_htb hd5 htb; *)
